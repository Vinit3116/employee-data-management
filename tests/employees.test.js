...
describe("Employee API", () => {
  ...
  test("GET /api/employees should return empty initially", async () => {
    ...
    expect(res.body.data).toEqual([]); // Access .data now
  });
  ...
  test("POST /api/employees should create employee", async () => {
    ...
    expect(res.body.data).toHaveProperty("id"); // Access .data
  ...
  test("PUT /api/employees/:id should update employee", async () => {
    ...
    expect(update.body.data.name).toBe("Bobby");
    expect(update.body.data.salary).toBe(40000);
  });
